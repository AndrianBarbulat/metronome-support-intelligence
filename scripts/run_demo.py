...
def _checked(params: dict[str, list[str]], name: str) -> bool:
    return _param(params, name).lower() in {"1", "true", "yes", "on"}
...
<div class='app'>
  ...
  <main class='main'>
    <header class='topbar'><div class='breadcrumbs'>Metronome Support Intelligence / <strong>{escape(breadcrumb)}</strong></div><div class='top-actions'><a class='btn small' href='/documentation'>Browse documentation</a><a class='btn small primary' href='/'>Ask a question</a></div></header>
...
<section class='status-grid'>
  ...
  <div class='status-card'><span class='dot'></span><div><div class='status-copy'>Documentation index ready</div><div class='status-sub'>{_fmt_count(m['chunks'])} chunks available</div></div></div>
...
def _documentation_search_tab(query: str) -> str:
...
def _documentation_articles_tab(params: dict[str, list[str]]) -> str:
...
def _documentation_chunks_tab(params: dict[str, list[str]]) -> str:
    page = _safe_int(_param(params, "page", "1"), default=1)
    page_size = 25
    ...
    contains_code = _checked(params, "contains_code")
    contains_table = _checked(params, "contains_table")
    contains_openapi = _checked(params, "contains_openapi")
...
def _documentation_openapi_tab(params: dict[str, list[str]]) -> str:
    method = _param(params, "method")
    endpoint = _param(params, "endpoint")
...
def _documentation_page(params: dict[str, list[str]]) -> str:
...
<section class='metric-grid'>
  ...
  <div class='card metric-card'><div class='metric-value'>{_fmt_count(metrics['openapi_blocks'])}</div><div class='metric-label'>OpenAPI blocks</div><div class='metric-foot'>{_fmt_count(metrics['code_blocks'])} code · {_fmt_count(metrics['tables'])} tables</div></div>
    ...
    if active_tab == "articles":
        tab_body = _documentation_articles_tab(params)
    elif active_tab == "chunks":
        tab_body = _documentation_chunks_tab(params)
    elif active_tab == "openapi":
        tab_body = _documentation_openapi_tab(params)
    else:
        tab_body = _documentation_search_tab(query)
...
def _article_detail_page(article_id: int, *, chunk_id: int | None = None) -> str:
    ...
    chunk_cards = "".join(
        _chunk_card({
            "id": c["id"],
            "page_id": article_id,
            "parsed_version_id": c["parsed_version_id"],
...
class AssistantHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if path == "/documentation":
            self._send_html(_documentation_page(params))
            return
        match = re.fullmatch(r"/documentation/articles/(\d+)", path)
        if match:
            chunk_id = _safe_int(_param(params, "chunk_id", "0"), default=0, minimum=0)
            self._send_html(_article_detail_page(int(match.group(1)), chunk_id=chunk_id or None))
            return
        if path == "/how-it-works":
            self._send_html(_how_it_works_page())
            return
        if path == "/testing":
            self._send_html(_testing_page())
            return
